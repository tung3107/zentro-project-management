import { useEffect } from 'react'
import { useAuthStore } from '../feature/auth/stores/authStore'
import api from './axiosClient'

export const usePermission = () => {
  const { accessToken, setPermission, setPermLoading, setIsChangePassword, setProjectPermissions, user } =
    useAuthStore()

  useEffect(() => {
    if (!accessToken) return
    ;(async () => {
      try {
        setPermLoading(true)
        const { data } = await api.get('/permission/me')
        const userData = data?.data?.data
        setPermission(userData)
        setIsChangePassword(data?.data?.is_change_password)

        if (user?.role_name === 'Member') {
          const { data } = await api.get(`/permission/project/all-permission`)
          const permissionData = data?.data
          setProjectPermissions(permissionData)
        }
      } catch {
        setPermission([])
      } finally {
        setPermLoading(false)
      }
    })()
  }, [accessToken, setPermLoading, setPermission, setIsChangePassword, setProjectPermissions, user])
}
