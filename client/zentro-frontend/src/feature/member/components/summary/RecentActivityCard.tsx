...
import { getActivityForProject } from '../../service/project.service'
...
export default function RecentActivityCard() {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        setLoading(true)
        // Fetch all activities without period filter, with higher limit
        const res = await getActivityForProject(projectId!, undefined, 1, 100)
  ...
  const getTaskTypeIcon = (typeValue: string) => {
  ...
  const renderActivityContent = (log: ActivityLog) => {
    // Handle sprint entity type
    if (log.entity_type === 'sprint') {
      const sprintName = log.sprint?.name || log.new_value_resolved?.name || log.entity_id

      switch (log.action_type) {
        case 'create':
          return (
            <span className='text-sm text-gray-800'>
              <span className='text-gray-700'>đã tạo sprint</span>{' '}
        ...
        case 'start':
          return (
            <span className='text-sm text-gray-800'>
              <span className='text-gray-700'>đã bắt đầu sprint</span>{' '}
              <span className='font-semibold text-blue-600'>{sprintName}</span>
            </span>
          )
        case 'complete':
          return (
            <span className='text-sm text-gray-800'>
              <span className='text-gray-700'>đã hoàn thành sprint</span>{' '}
              <span className='font-semibold text-blue-600'>{sprintName}</span>
            </span>
          )
        default:
          return (
            <span className='text-sm text-gray-800'>
              <span className='text-gray-700'>đã thay đổi sprint</span>{' '}
              <span className='font-semibold text-blue-600'>{sprintName}</span>
            </span>
          )
      }
    ...
    // Handle task entity type
    const taskType = log.task?.type || log.new_value_resolved?.type
    const taskIcon = taskType ? getTaskTypeIcon(taskType) : null
    const taskId = log.task?.task_id || log.entity_id
    const taskTitle = log.task?.title || log.new_value_resolved?.title

    const taskLink = (
      <Link
        to={`/member/projects/${log.project_id}/backlog?task=${taskId}`}
        className='inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 hover:underline font-medium transition-colors align-middle'
      >
        {taskIcon}
        <span>{taskId}</span>
      </Link>
    ...
    switch (log.action_type) {
      case 'create':
        return (
          <span className='text-sm text-gray-800'>
            <span className='text-gray-700'>đã tạo công việc</span> {taskLink}
            {taskTitle && (
              <>
                {' '}
                <span className='text-gray-900 font-medium truncate max-w-[300px]' title={taskTitle}>
                  {taskTitle}
                </span>
              </>
            )}
      ...
      case 'update': {
        ...
        if (!changes || changes.length === 0) {
          return (
            <span className='text-sm text-gray-800'>
              <span className='text-gray-700'>đã cập nhật</span> {taskLink}
            </span>
          )
        }
  ...
  return (
    <ChartWrapper className='col-span-3 flex flex-col items-start justify-start pt-4 relative'>
      <Header className='text-left mb-1'>Hoạt động gần đây</Header>
      ...
      {loading ? (
        ...
        <div className='absolute inset-0 flex flex-col items-center justify-center text-center'>
          <img src='/Not Found.png' alt='No activity' className='w-[180px] h-[180px] object-contain opacity-90' />
          <h1 className='text-base font-semibold text-gray-800 max-w-[280px] leading-relaxed'>
            Không có hoạt động gì.
          </h1>
        </div>
      ) : (
        <div className='w-full flex-1 overflow-y-auto pr-2' style={{ maxHeight: '300px' }}>
                    ...
                    >
                      ...
                      <div className='flex-1 min-w-0'>
                        <div className='flex flex-col gap-0.5'>
                          ...
                          <span className='text-xs text-gray-500 mt-0.5'>{formatTime(log.created_at)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
