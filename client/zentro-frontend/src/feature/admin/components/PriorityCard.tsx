...
// Define labels and color mapping (fixed in FE)
...
export default function PriorityCard({ percentList }: PriorityCardProps) {
  ...
  return (
    <ChartWrapper className='col-span-2 pt-[40px]! pb-[0px]!'>
      ...
      <div style={{ width: '100%' }}>
        {percentList.map((percent, idx) => (
          <ProgressBarContainer key={PRIORITY_LABELS[idx] || idx}>
            ...
            <ProgressTrack>
              ...
              >
                <ProgressFill color={PRIORITY_COLORS[idx]} percent={percent || 0} style={{ zIndex: 2 }}>
                  <ProgressPercent>{percent || 0}%</ProgressPercent>
                  <BarTooltip show={hoverBarIndex === idx} percent={percent || 0}>
                    {percent || 0}%
                  </BarTooltip>
                </ProgressFill>
              </ProgressFillWrapper>
