...
import { projectStatAPI } from '../service/project.service'
...
const iconMap: Record<string, JSX.Element> = {
  total_projects: <Folder className='text-blue-500' size={22} />,
  projects_in_progress: <Play className='text-orange-500' size={22} />,
  projects_completed: <CheckCircle className='text-green-500' size={22} />,
  tasks_done_ratio: <Target className='text-green-500' size={22} />,
  tasks_overdue: <AlertTriangle className='text-red-500' size={22} />,
  online_members: <Users className='text-gray-500' size={22} />
}

export default function ProjectStats({ reloadKey }: { reloadKey: number }) {
  const [stats, setStats] = useState<any[]>([])

  useEffect(() => {
    setStats([])
    async function fetchData() {
      const data = await projectStatAPI()
      setStats(data.data)
    }
    fetchData()
  }, [reloadKey])
          ...
          <div className='flex justify-between items-center'>
            ...
            {item.trend && (
              <div className='flex flex-col items-end'>
                <span className={`text-xs font-medium text-${item.trendColor}-500`}>{item.trend}</span>
