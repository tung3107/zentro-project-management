...
import { getUserProfile } from '../../member/service/user.service'
...
export default function Profile() {
  ...
  const isOwnProfile = userId === currentUser?.user_id || !userId // If no userId param, assume own profile

  useEffect(() => {
    const fetchUser = async () => {
      try {
        setLoading(true)
        const idToFetch = userId || currentUser?.user_id
        if (idToFetch) {
